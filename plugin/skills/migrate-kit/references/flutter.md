...
### Flutter 3.22 → 3.27
- **Wide gamut Color API change** — Color now stores alpha as a floating-point value. `.withOpacity()` and `.opacity` deprecated:
  ```dart
  // Before
  ...
  final alpha = color.a;
  ```
- **Cupertino widgets** updated with rounded, almost-square shapes to match native iOS.
- **Material 3 TabBar** improvements with better interactivity and customization.
- **Property Editor** in Flutter DevTools for inspecting and editing widget properties.

### Flutter 3.27 → 3.29
- **Flutter Gradle plugin** — Script-based application removed (deprecated since 3.19). Must use the declarative Kotlin-based plugin:
  ```kotlin
  ...
  ```
- **Impeller on Android** — Devices without functional Vulkan fall back to Impeller on OpenGLES instead of Skia.
- **Several packages discontinued**: `ios_platform_images`, `css_colors`, `palette_generator`, `flutter_image`, `flutter_adaptive_scaffold`, `flutter_markdown`.

### Flutter 3.29 → 3.32
- **Hot reload on web** (experimental).
- **Impeller floor** — API 29+ uses Impeller, API 28 and below reverted to Skia for stability.
- **`ThemeData.indicatorColor`** removed — use `TabBarThemeData.indicatorColor`.
- **`SystemContextMenuController.show`** deprecated.

### Flutter 3.32 → 3.38
- **Dart dot shorthands** — Reduce boilerplate by omitting types Dart can infer:
  ```dart
  // Before
  MainAxisAlignment.start
  // After
  .start  // when type is inferable from context
  ```
- **UIScene lifecycle** support required by Apple — mandatory code migration for iOS apps.
- **`AssetManifest.json`** no longer generated by default.
- **Flutter SDK version file** replaced with `flutter.version.json` in `bin/cache`.
- **SnackBar with action** no longer auto-dismisses.

### Flutter 3.38 → 3.41
- **Bounded blur** rendering — Eliminates color bleeding at edges of translucent widgets using `BackdropFilter` via Impeller improvements.
- **Embedded Flutter views** can now auto-resize based on content in native apps.
- **Synchronous Image Decoding** with `decodeImageFromPixelsSync` for same-frame texture generation.
- **High Bitrate Textures** support (up to 128-bit float) for GPU-accelerated photo filters.
- **Platform-specific assets** — Specify which platforms an asset should be bundled for in `pubspec.yaml`, reducing app size.
